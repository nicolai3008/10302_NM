...
atoms = read("../"+"Materials/1MnI2-1.xyz")
M = [[2,1,0],[1,2,0],[0,0,1]]
...
energies = np.load('../MnI2/data_600.0_6.0_6.0.npz')['energies']
minimum = np.argmin(energies)
calc = GPAW(f'../MnI2/Band/gsq-{minimum:02}.gpw')
with open(f'../MnI2/Band/gsq-{minimum:02}.txt') as f:
    lines = f.readlines()
    f.close()
